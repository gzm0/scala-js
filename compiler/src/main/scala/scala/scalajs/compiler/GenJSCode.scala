...
abstract class GenJSCode extends plugins.PluginComponent
  ...
  class JSCodePhase(prev: Phase) extends StdPhase(prev) {
    ...
    private def genStringCall(tree: Apply): js.Tree = {
      ...
      // Filter members of implementation class for matching member
      val implMembers = for {
        mem <- RuntimeStringImplClass.tpe.members
        if mem.name == sym.name
        // Deconstruct method type (drop non-methods)
        MethodType(params, resultType) = mem.tpe
        if params.size >= 1
        // Reconstruct original method type (drop first argument)
        orgType = MethodType(params drop 1, resultType)
        // Match reconstructed type
        if sym.tpe.matches(orgType)
      } yield mem
      ...
      if (implMembers.isEmpty) {
        ...
        js.ApplyMethod(
          encodeModuleSym(RuntimeStringImplClass),
          encodeMethodSym(implMembers.head),
          receiver :: args)
        }
    }
