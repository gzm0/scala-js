...
abstract class GenJSCode extends plugins.PluginComponent
  ...
  class JSCodePhase(prev: Phase) extends StdPhase(prev) {
      ...
      def gen(tree: Tree): Unit = {
        tree match {
          ...
          case dd: DefDef =>
            ...
            // Generate a method proxy with return type Any for reflective calls
            // if required
            generatedMembers ++= ReflCallProxyGen.genReflCallProxy(dd.symbol)
    ...
    object ReflCallProxyGen {

      /** Denote the state of a symbol for proxy generation */
      sealed abstract class SymState { val generate = false }
      /** Should generate a proxy */
      case object Generate extends SymState {
        override val generate = true
      }
      /**
       *  Proxy already exists or is not required
       *  (inherited or more general method, constructors)
       */
      case object NoGenerate extends SymState
      /**
       *  There are conflicting methods. Impossible to generate a proxy
       *
       *  If multiple overloaded methods only differ in return type, we cannot
       *  disambiguate at the call site. ScalaJVM fails on this.
       */
      case class Conflict(
          origin: Symbol,
          conflicting: List[Symbol]
          ) extends SymState
       ...
       * generate a method with most general return type (Any) to be used in
       * reflective calls if no such method already exists. (Sometimes earlier
       * phases do generate them)
       *
       * Further, note that this will fail if there are non-trivial overloads that
       * only differ in return type (allowed on JVM backend and Scala.js backend
       * but not in the Scala language itself).
       */
      def genReflCallProxy(sym: Symbol): Option[js.Tree] = {
        ...
        symState(sym) match {
          ...
          case NoGenerate =>
            None
          case Conflict(orig, overs) =>
            val sb = new StringBuilder(
                s"""Ambiguous overload. Cannot generate return any proxy.
                   |Offending symbol: ${sym.fullName}
                   |With type: ${sym.tpe}
                   |Alternatives:
                   |""".stripMargin)

            for (a <- overs) {
              sb.append(s"""Symbol    = ${a.fullName}
                           |  paramss = ${a.paramss}
                           |  type    = ${a.tpe}
                           |
                           |""".stripMargin)
            }
            currentCUnit.warning(pos, sb.toString)
            None
        }
      }

      /** get state of a symbol */
      def symState(sym: Symbol): SymState =
        sym.attachments.get[SymState] getOrElse {
          val state = calcSymState(sym)
          sym.attachments.update(state)
          state
        }

      private def calcSymState(sym: Symbol) = {
        assert(sym.isMethod)

        // First trivial checks if we actually need a proxy
        if (sym.isConstructor ||
          ...
          // Inspect other existing methods generated by the compiler
          val pars = sym.tpe.params
          ...
          // Filter other methods. We want:
          // - Match in name and parameters
          // - If it's in a parent, to have generated a proxy
          // The goal is to figure out, whether a proxy needs to be generated or
          // if a superclass or a more general method has already taken care of it
          val overs = alts filter { s =>
            ...
            !(sym.overriddenSymbol(s.owner) == s)
            //!(sym.tpe.resultType <:< s.tpe.resultType)
            //!sym.allOverriddenSymbols.contains(s)
            //!(sym.tpe <:< s.tpe)
/*            !alts.exists(a =>
              sym.owner.baseClasses.filter(_ != sym.owner).exists(base =>
                a.overriddenSymbol(base) == s))*/
          }

          // Check if we have only one method. After erasure, we can have two
          // different methods whose signature only differs in return type
          if (!overs.isEmpty)
            Conflict(sym, overs)
          else if (sym.allOverriddenSymbols.exists(s => symState(s).generate))
            NoGenerate
          else
            Generate
    ...
    private def genApplyDynamic(tree: ApplyDynamic,
      ...
      val apply = js.ApplyMethod(instance,
