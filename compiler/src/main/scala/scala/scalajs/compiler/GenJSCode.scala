  ...
  class JSCodePhase(prev: Phase) extends StdPhase(prev) {
      ...
      def gen(tree: Tree): Unit = {
        tree match {
          ...
          case dd: DefDef =>
            ...
            // if required
            generatedMembers ++= genRetAnyProxy(dd.symbol)
    ...
    def genRetAnyProxy(sym: Symbol) = {
      ...
      if (sym.isDeferred    ||
        ...
        if (retTps.exists(_.typeSymbol == ObjectClass)) {
          ...
          // there is a more general method. use its proxy
          None
        } else if ()
          overs exists {
        ...
        overs match {
          ...
          case x :: Nil if x.tpe.resultType.typeSymbol == ObjectClass =>
    ...
    private def genApplyDynamic(tree: ApplyDynamic,
      ...
      val apply = js.ApplyMethod(instance,
          encodeMethodSym(sym, retAny = true), arguments)
